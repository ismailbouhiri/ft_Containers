...
#include "../Vector/vector.hpp" // your Vector path.  // path to ft::pair.
...
void testOperations()
{
    ...
    std::cout << "\t\033[1;37m[-------------------- [" << std::setw(40) << std::left << " lower_bound method " << "] --------------------]\t\t\033[0m";
    {
        bool cond;
        /*---------------------------------- time limit test --------------------------------------------*/
        {
            ...
            start = get_time();
            res = m.lower_bound(1e2)->first;
            end = get_time();
            diff = end - start;
            diff = (diff) ? (diff * TIME_FAC) : TIME_FAC;

            ualarm(diff * 1e3, 0);
            ft_res = ft_m.lower_bound(1e2)->first;
            ualarm(0, 0);
            cond = ft_res == res;
        }
        std::map<int, std::string> m;
              ...
              << "] --------------------]\t\t\033[0m";
    {
        ...
        /*---------------------------------- time limit test --------------------------------------------*/
        {
            ...
            start = get_time();
            res = m.equal_range(1e2);
            end = get_time();
            diff = end - start;
            diff = (diff) ? (diff * TIME_FAC) : TIME_FAC;

            ualarm(diff * 1e3, 0);
            ft_res = ft_m.equal_range(1e2);
            ualarm(0, 0);
...
void testRetionalOperators()
{
    ...
    /*---------------------------------- time limit test --------------------------------------------*/
    {
        ...
        ualarm(diff * 1e3, 0);
        // std::cout << "ft_res = " << ft_res << "res = " << res << std::endl;
        ft_res = ft_m > ft_m1;
        ualarm(0, 0);
    }
    EQUAL(((foo > bar) == (ft_foo > ft_bar)) && (res == ft_res));
